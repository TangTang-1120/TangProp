...
from net_http import httpx_client
...
class WebSearchTool(BaseTool):
    ...
    async def execute(self, arguments: Dict[str, Any]) -> ToolResult:
        ...
        # 1) Try DuckDuckGo HTML first
        try:
            async with httpx_client(timeout=15) as client:
                ...
                import re as _re
                # Match title anchors and their nearby snippets
                pattern = (
                ...
                results = []
                for i, (title, snippet) in enumerate(matches[:num]):
        ...
        # 2) Fallback to Wikipedia summary (often useful for known topics)
        try:
            import urllib.parse
            async with httpx_client(timeout=12) as client:
                resp = await client.get(
                ...
                if resp.status_code == 200:
                    ...
                    if hits:
                        lines = []
                        for i, h in enumerate(hits[:num]):
                            # Strip HTML tags from snippet
                            import re as _re
                            snippet = _re.sub(r'<[^>]+>', '', h.get("snippet", ""))
                            lines.append(f"{i+1}. {h.get('title', '')}\n   {snippet}")
                        return ToolResult(
                            success=True,
                            output="\n\n".join(lines),
                            metadata={"query": query, "result_count": len(lines), "source": "wikipedia"},
                        )
        except Exception as e:
            print(f"[WebSearch] Wikipedia fallback failed: {e}")

        # 3) Last resort: use the model's own knowledge — but be honest about it
        return ToolResult(
            success=False,
            output=(
                f"实时搜索暂时不可用（搜索引擎返回了空结果）。"
                f"无法实时获取 '{query}' 的最新信息。"
                f"请基于你自己的知识给出回答，并明确标注信息可能不是最新的。"
            ),
            metadata={"query": query, "result_count": 0, "source": "fallback"},
        )
...
class WebFetchTool(BaseTool):
    ...
    async def execute(self, arguments: Dict[str, Any]) -> ToolResult:
        url = arguments["url"]
        try:
            async with httpx_client(timeout=20, follow_redirects=True) as client:
                resp = await client.get(
                    url,
...
class DoubaoTtsTool(BaseTool):
    ...
    async def execute(self, arguments: Dict[str, Any]) -> ToolResult:
        ...
        try:
            import uuid
            async with httpx_client(timeout=60) as client:
...
class DoubaoAsrTool(BaseTool):
    ...
    async def execute(self, arguments: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            import uuid
            async with httpx_client(timeout=120) as client:
